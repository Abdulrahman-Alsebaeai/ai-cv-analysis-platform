...
export async function POST(req: Request) {
  ...
  const fullName = cleanFormValue(body.full_name || metadata.full_name || user.email || "User");
  const phone = cleanFormValue(body.phone ?? metadata.phone ?? "") || null;
  const role = normalizeRole(body.role ?? metadata.role ?? "applicant");
  const companyId = cleanFormValue(metadata.company_id ?? "") || null;
